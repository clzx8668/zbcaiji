...
| ✅ 可采集 | 上海政府采购网 | 政采云适配器（`sites/zcy.py`）：浏览器签名搜索 + 时间筛选 + 详情直连 HTTP，已跑通 |
| ✅ 可采集 | 浙江政府采购网 | 同一政采云适配器复用，时间筛选收敛到近 1 月，详情直连补全，已跑通 |
| ✅ 可采集 | 中国石化电子招投标平台 | `sites/sinopec.py`：首页最新公告流 + 本地关键词过滤（详情需供应商登录，列表级入库），已跑通 |
| ✅ 可采集 | 山东能源集团招标投标交易平台 | `sites/minegoods.py`：关键词搜索 URL 直接翻页（详情需登录，列表级入库），已跑通，近期约 40+ 条 |
| ✅ 可采集 | 中海油采办业务管理与交易系统 | `sites/cnooc.py`：旧站 buy.cnooc.com.cn 已故障，新平台 bid.cnooc.com.cn API 直连（日期范围全量 + 本地过滤 + 详情正文），已跑通 |
| ✅ 可采集 | 中国华能集团电子商务平台 | `sites/chng.py`：JS 反爬（412）适配 + 关键词搜索（标题）+ 详情正文，已跑通 |
| ⚠️ 需人工验证码 | 中国石油招标投标网 | `sites/cnpc.py`：SPA + RSA 加密 + 图形验证码；无头模式会明确失败，需交互模式输入一次验证码后会话自动保存 |
...
| 📋 已入库 | 中煤、国家能源集团 | 企业招标平台，多为登录/反爬，待适配 |
| 📋 已入库 | 北京/天津/江苏/山东/广东/福建/河北/辽宁政府采购网 | 东部十省市地方招标网站（上海/浙江已完成；北京、天津、江苏等为各自独立系统，待适配） |
...
| 中石油交互验证码 | `python run.py crawl --site "中国石油招标投标网"`（不加 `--no-interact`，首次输入验证码，会话自动保存） |
